...
def extract(query, choices, score_cutoff=0, limit=5):
    results = []
    for i in choices:
        if i[0]:
            score = (get_jaro_distance(query, i[0]))
            data = (i[1], score)
            if score >= score_cutoff:  # type: ignore
                results.append(data)
    return sorted(results, key=lambda j: j[1], reverse=True)[:limit]


def extractOne(query, choices, score_cutoff=0):
    best_list = extract(query, choices, score_cutoff)
